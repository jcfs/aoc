...
def check_point(p):
  ...
  vm.run(p[0])
  vm.run(p[1])
  return vm.get_output()[-1] == 1

c = 0
for x in range(50):
  for y in range(50):
    c += 1 if check_point((x, y)) else 0

print("p1", c)

offset = 0
for y in range(99, 2000):
  for x in range(offset, 2000):
    if not check_point((x,y)): 
      continue
    offset = x
    if not check_point((x+99, y-99)):
      break

    print("p2", x*10000+(y-99))
    exit()
